...
public class RobotMove extends SendUDP {
	...
	public RobotMove(int xtar, int ytar, int ztar, int phitar, int thetatar, int[] tool, int speedin) {
		...
		for (int i = 0; i < 3; i++) {
			...
			case 0:
				...
				this.angle[i] = phitar; // pitch in 1 degree
				break;
			case 1:
				this.coordinate[i] = ytar; // Y in 1 mm
				this.angle[i] = thetatar; // yaw in 1 degree
				break;
			case 2:
	...
	public boolean isDone() {
		if (this.displacement[5] == 9999 && this.displacement[6] == 9999) {
			...
			robotDisplacement();
			if (Math.abs(this.displacement[0]) >= 1000 && Math.abs(this.displacement[1]) >= 1
					&& Math.abs(this.displacement[2]) >= 1000 && Math.abs(this.displacement[3]) >= 1
					&& Math.abs(this.displacement[4]) >= 1000 && Math.abs(this.displacement[7]) >= 1) {
				this.isDone = false;
	...
	private boolean isDoneRec() {
		if (this.displacement[5] == 9999 && this.displacement[6] == 9999) {
			...
			robotDisplacement();
			if (Math.abs(this.coordinate[0] - this.displacement[2]) >= 1
					|| Math.abs(this.coordinate[1] - this.displacement[3]) >= 1
					|| Math.abs(this.coordinate[2] - this.displacement[4]) >= 1) {
				this.isDone = false;
			} else {
				this.isDone = true;
			}
			System.out.printf("Remaining Displacement  X : %d mm  Y : %d mm Z : %d mm.",
					(this.coordinate[0] - this.displacement[2]),
					(this.coordinate[1] - this.displacement[3]),
	...
	private boolean isDoneAng(int index) {
		...
		if (this.displacement[5] == 9999 && this.displacement[6] == 9999) {
			...
			if (index == 0) {
				// Set pitch to 0
				if (Math.abs(0 - this.displacement[6]) >= 1) {
					this.isDone = false;
				} else {
					this.isDone = true;
				}
				...
				if (Math.abs(this.angle[1] - this.displacement[6]) >= 1) {
					this.isDone = false;
				} else {
					this.isDone = true;
				}
				...
				if (Math.abs(this.angle[0] - this.displacement[5]) >= 1) {
					this.isDone = false;
				} else {
					this.isDone = true;
				}
			...
			System.out.println("Remaining Angular Pitch : " + (this.angle[0] - this.displacement[5])
					+ " degrees Yaw : " + (this.angle[1] - this.displacement[6]) + " degrees.");
			return this.isDone;
		}
	}
	...
	public void robotDisplacement() {
		...
		if (toolnow == null) {
			...
			System.out.println("Displacement X : " + this.displacement[2]+ " mm, Y : "
					+ this.displacement[3] + " Z : " + this.displacement[4] + " mm, Pitch : "
					+ this.displacement[5] + " degrees, Yaw : " + this.displacement[6]+ " degrees.");
		}
	}
	...
	public void move() throws Exception {
		...
		try {
			...
			} else if (this.coordinate[0] >= 1 || this.coordinate[1] >= 1 || this.coordinate[2] >= 1) {
				int loopcnt = 0;
				while (!(isDoneRec())) {
			...
			} else if (this.angle[0] >= 1 || this.angle[1] >= 1) {
				// Assign angle is greater than 1 degree
				int loopcnt = 0;
				this.isPitch = true;
				while (!(isDoneAng(0))) {
