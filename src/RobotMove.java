...
public class RobotMove extends SendUDP {
	...
	private boolean isDoneRec() {
		if (this.displacement[5] == 9999 && this.displacement[6] == 9999) {
			...
			robotDisplacement();
			if (Math.abs(this.coordinate[0] - this.displacement[2]) >= 10		// X displacement greater than 10 mm
				|| Math.abs(this.coordinate[1] - this.displacement[3]) >= 10	// Y displacement greater than 10 mm
				|| Math.abs(this.coordinate[2] - this.displacement[4]) >= 10){	// Z displacement greater than 10 mm
				this.isDone = false;
			} else {
				this.isDone = true;
			}
	...
	private boolean isDoneAng(int index) {
		...
		if (this.displacement[5] == 9999 && this.displacement[6] == 9999) {
			...
			if (index == 0) {
				...
				if (Math.abs(this.angle[1] - this.displacement[6]) >= 10) {	// 0.1 degree
					this.isDone = false;
				} else {
					this.isDone = true;
				}
				...
				if (Math.abs(this.angle[0] - this.displacement[5]) >= 10) {	// 0.1 degree
					this.isDone = false;
				} else {
					this.isDone = true;
				}
	...
	public void robotDisplacement() {
		...
		if (toolnow == null) {
			...
			System.out.println("Displacement X : " + this.displacement[2]+ " mm, Y : "
					...
					+ this.displacement[5]/100 + " degrees, Yaw : " + this.displacement[6]/100 + " degrees.");
			*/
		}
	}
	...
	public void move() throws Exception {
		...
		try {
			...
			if (this.moveTool) {
				...
				// 10 mm or 0.1 degree
			} else if (this.coordinate[0] >= 10 || this.coordinate[1] >= 10 || this.coordinate[2] >= 10) {
				int loopcnt = 0;
				while (!(isDoneRec())) {
			...
			} else if (this.angle[0] >= 10 || this.angle[1] >= 10) {
